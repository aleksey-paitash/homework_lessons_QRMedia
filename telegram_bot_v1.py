from misc_bot import token  # My name_bot = Aleksey_test_ORmedia_bot
...
def pogoda(id, text):
    ...
    html = requests.get(url).text
    soup = BeautifulSoup(html, 'lxml')
    pogoda = soup.find('div', class_="temp fact__temp fact__temp_size_s").find('span', class_="temp__value").text
    return pogoda
    ...
    while True:
        ...
        if answer != None:
            ...
            if (text.split('_')[0])[1:4].upper() in code_id and len(text) >= 13:
                send_message(id, kurs_date(id, text))
            if text == '/anekdot':
                send_message(id, anekdot(id, text))
            if text == '/afisha':
                send_message(id, afisha(id, text))
            if text == '/pogoda':
                send_message(id, pogoda(id, text))
        else:
            continue
